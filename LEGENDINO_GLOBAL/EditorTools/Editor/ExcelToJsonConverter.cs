...
public class ExcelToJsonConverter : Editor
{
    private static ExcelDataToJsonConverter m_converter;
    [MenuItem("Assets/ConvertExcelToJson")]
    public static void ConvertExcelToJson()
    {
        if(m_converter == null)
            m_converter = new ExcelDataToJsonConverter();
        ...
        if(selectedObjects.Length <= 1)
        {
            ...
            {
                //m_converter.ConvertExcelFileToJson(path, Utils.GetAppendedSentence(Application.dataPath, "/", "GameDatas/JsonData"));
				m_converter.ConvertExcelFileToJson(path, Utils.GetAppendedSentence(Application.dataPath, "/", "GameDatas"));
            }
        }
...
public class ExcelDataToJsonConverter
{
	...
	private string GetSpreadSheetJson(DataSet excelDataSet, string sheetName)
	{
		...
		json = json.Replace(".0,", ",");
		json = json.Replace(".0}", "}");
		Dictionary<string, object>[] deserializeObjects = JsonConvert.DeserializeObject<Dictionary<string, object>[]>(json);

		List<string> typeChangedKeys = new List<string>();

		Dictionary<string, Type> rowDataTypes = new Dictionary<string, Type>();
		foreach(KeyValuePair<string, object> pair in deserializeObjects[0])
		{
			rowDataTypes.Add(pair.Key, pair.Value.GetType());
		}

		foreach(Dictionary<string, object> data in deserializeObjects)
		{
			foreach(string keyName in data.Keys)
			{
				Debug.Log(keyName);
				// if( data[keyName] == null)
				// 	Debug.Log("khj!!         "+keyName);
				Type dataType = data[keyName].GetType();
				Type beforeDataType = rowDataTypes[keyName];
				if(dataType != beforeDataType)
				{
					// Debug.Log(keyName +"//"+dataType+"//"+beforeDataType);
					if(!typeChangedKeys.Contains(keyName))
						typeChangedKeys.Add(keyName);
				}
				rowDataTypes[keyName] = dataType;
			}
		}

		if(typeChangedKeys.Count <= 0)
			return JsonConvert.SerializeObject(deserializeObjects, Formatting.Indented);

		foreach(Dictionary<string, object> data in deserializeObjects)
		{
			foreach(string keyName in typeChangedKeys)
			{
				object cellValue = data[keyName];
				if(cellValue is Int64)
					data[keyName] = (Double)(Int64)cellValue;
			}
		}

		json = JsonConvert.SerializeObject(deserializeObjects, Formatting.Indented);
		return json;
	}
